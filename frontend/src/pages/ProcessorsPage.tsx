...
export const ProcessorsPage: React.FC = () => {
  ...
  const fetchProcessors = async () => {
    ...
    setError(null);
    try {
      const data = await apiService.getProcessors();
      setProcessors(data);
    } catch (err: any) {
      setError(err?.message || 'Failed to fetch hardware profiles from backend API.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const displayList = processors.length > 0 ? processors : PROCESSOR_PROFILES;

  const filteredProcessors = displayList.filter((p) => {
    const matchesSearch =
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Technical hardware profiles for STM32F0, STM32F4, STM32H7, nRF52840, HiFive1, and ESP32 target platforms
            </p>
          </div>
        </div>
      </Card>
      ...
      {loading ? (
        <Card className="p-8">
          <LoadingSpinner label="Loading target hardware profiles from backend API..." />
        </Card>
      ) : error && processors.length === 0 ? (
        <ApiErrorState
          title="Hardware Profiles API Offline"
          message={error}
          onRetry={fetchProcessors}
        />
      ) : (
