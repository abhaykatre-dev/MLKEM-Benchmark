/**
 * benchmark.c — HiFive1 Profile (16 KB SRAM, 48 MHz TIM2)
 *
 * Research-Grade ML-KEM Benchmark Driver for HiFive1 Profile (NIST FIPS 203)
 * Timing: TIM2 Microsecond Hardware Precision
 */
...
void benchmark_run_all(void)
{
    ...
    uart_print("  Target Processor: SiFive HiFive1 Profile (RISC-V 320 MHz)\r\n");
    uart_print("  Clock Speed:      48 MHz Timer | 16 KB SRAM\r\n");
    uart_print("  Sampling Count:   N = 5 Iterations\r\n");
    uart_print("=======================================================\r\n");
    ...
    uart_print("\r\nExecuting ML-KEM-768 Benchmark (OOM Check)...\r\n");
    benchmark_mlkem768_oom(&r768);
    benchmark_print(&r768);

    uart_print("\r\nExecuting ML-KEM-1024 Benchmark (OOM Check)...\r\n");
    benchmark_mlkem1024_oom(&r1024);
    ...
    uart_print("\r\n=======================================================\r\n");
    uart_print("  HiFive1 BENCHMARK SUITE COMPLETE!\r\n");
    uart_print("=======================================================\r\n");
}
...
void benchmark_print(BenchmarkResult *r)
{
    ...
    if (r->decap_ok == 2) {
        uart_print("  Status : OOM [Out Of Memory: Requires >16 KB SRAM]\r\n");
        return;
    }
    ...
    snprintf(buf, sizeof(buf), "  Status : %s\r\n",
        r->decap_ok == 1 ? "PASS [Shared Secret Match]" : "FAIL [Mismatch]");
    uart_print(buf);
}
...
void benchmark_print_csv_row(BenchmarkResult *r)
{
    char buf[240];
    if (r->decap_ok == 2) {
        snprintf(buf, sizeof(buf), "%s,OOM,OOM,OOM,OOM,OOM,OOM,0,0,0,OOM\r\n", r->variant);
    } else {
        snprintf(buf, sizeof(buf),
